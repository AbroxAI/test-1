// ultimate-realism-full-v9.2-merged-with-images.js
(function () {
  'use strict';
  ...
  function generateComment(persona, lastTimestamp = new Date()) {
    let poolFuncs = [
      ...
      () =>
        `Anyone trading ${smartPick(ASSETS, persona.memory)} on ${smartPick(
          BROKERS,
          persona.memory
        )}?`,
      () =>
        `Result was ${smartPick(RESULT_WORDS, persona.memory)} on ${smartPick(
          ASSETS,
          persona.memory
        )}`,
    ];
    ...
    if (Math.random() < 0.2) {
      text =
        smartPick(REPLY_TEMPLATES, persona.memory) +
        " — " +
    ...
    // IMAGE OPTION — text always remains
    let image = null;
    let caption = null;
    if (Math.random() < 0.15) {
      const imgIndex = 1 + Math.floor(Math.random() * 50);
  ...
  async function processQueue() {
    ...
    while (interactionQueue.length > 0) {
      ...
      const { persona, text, parentText, parentId, meta, image, caption } = inter;
      const opts = { image, caption };
  ...
  console.log("✅ Ultimate Realism Engine
